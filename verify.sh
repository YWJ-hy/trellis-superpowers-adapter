...
assert_contains "$BRAINSTORM_FILE" 'normalize.md' 'brainstorm normalization artifact'
assert_contains "$BRAINSTORM_FILE" 'memorandum.md' 'brainstorm memorandum artifact'
...
assert_contains "$BRAINSTORM_FILE" 'fixed core columns' 'brainstorm fixed core columns rule'
assert_contains "$BRAINSTORM_FILE" 'AI may extend but must not replace core columns' 'brainstorm extension without replacement rule'
assert_contains "$BRAINSTORM_FILE" 'frontend field core table' 'brainstorm frontend field core table rule'
assert_contains "$BRAINSTORM_FILE" 'Backend API core table minimum columns' 'brainstorm backend api core table rule'
assert_contains "$BRAINSTORM_FILE" 'Entity / data structure core table minimum columns' 'brainstorm entity core table rule'
assert_contains "$BRAINSTORM_FILE" 'Business rule core table minimum columns' 'brainstorm business rule core table rule'
assert_contains "$BRAINSTORM_FILE" 'Source Anchor' 'brainstorm source anchor core column'
assert_contains "$SPECIFY_FILE" 'active Trellis task' 'specify active task requirement'
assert_contains "$SPECIFY_FILE" 'normalize.md' 'specify normalization input'
assert_contains "$SPECIFY_FILE" 'memorandum.md' 'specify memorandum input'
assert_contains "$SPECIFY_FILE" 'non-committed' 'specify memo filtering rule'
assert_contains "$SPECIFY_FILE" 'prd.md' 'specify PRD target'
assert_contains "$SPECIFY_FILE" 'Critical Details to Preserve' 'specify critical details section'
assert_contains "$SPECIFY_FILE" 'Spec Review Gate' 'specify review gate section'
assert_contains "$SPECIFY_FILE" 'frontend projects' 'specify frontend ui preservation rule'
assert_contains "$SPECIFY_FILE" 'table columns' 'specify ui detail examples'
...
assert_contains "$SPECIFY_FILE" 'fixed core columns' 'specify fixed core column carry-forward rule'
assert_contains "$SPECIFY_FILE" 'normalized backend inputs, outputs, enums, validations, errors, side effects, permissions, and entity constraints remain visible' 'specify backend contract visibility rule'
assert_contains "$REPLAN_FILE" 'refresh the relevant fixed core table' 'replan fixed core table refresh rule'
...
assert_contains "$CLARIFY_FILE" 'Critical Details to Preserve' 'clarify critical detail integration'
assert_contains "$CLARIFY_FILE" 'Spec Review Gate' 'clarify review gate integration'
...
assert_contains "$PLAN_FILE" 'trace.md' 'plan trace artifact'
assert_contains "$PLAN_FILE" 'Requirement Coverage Map' 'plan coverage map section'
assert_contains "$PLAN_FILE" 'Proof Obligations' 'plan proof obligations section'
assert_contains "$PLAN_FILE" 'Relevant Parent Context Slice' 'plan relevant parent context slice section'
...
assert_contains "$EXECUTE_FILE" 'trace.md' 'execute trace usage'
assert_contains "$EXECUTE_FILE" 'Relevant Parent Context Slice' 'execute child-first parent slice usage'
assert_contains "$EXECUTE_FILE" 'spec-compliance review' 'execute spec compliance review'
assert_contains "$EXECUTE_FILE" 'UI controls/components' 'execute frontend ui preservation review'
assert_contains "$EXECUTE_FILE" 'Proof Log' 'execute proof log updates'
...
assert_contains "$EXECUTE_FILE" 'child `Relevant Parent Context Slice`' 'execute child-first slice review order'
...
assert_contains "$REPLAN_FILE" 'normalize.md' 'replan normalization refresh handling'
assert_contains "$REPLAN_FILE" 'memorandum.md' 'replan memorandum refresh handling'
assert_contains "$REPLAN_FILE" 'trace.md' 'replan trace history handling'
...
assert_contains "$REPLAN_FILE" 'without silently overwriting prior proof history' 'replan preserve proof history'
...
assert_contains "$START_FILE" 'normalize.md' 'start interop normalization-aware parent resume'
assert_contains "$START_FILE" 'memorandum.md' 'start interop memorandum-aware parent resume'
...
assert_contains "$START_FILE" 'resume with `/trellis-sp:specify`' 'start interop specify resume on normalized parent'
...
assert_contains "$INTEGRATION_CN_FILE" '最终校验前再切回 parent' 'integration doc execute current-task switching'
