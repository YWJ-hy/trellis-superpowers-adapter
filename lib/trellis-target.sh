...
render_start_interop_block() {
  ...
  printf '%s\n' '- Managed parent task: describe it as a Superpowers-managed parent task, read its `normalize.md` and `memorandum.md` when present, then read its `prd.md` plus any task-local `info.md`, inspect `task.json.meta.trellis_sp.last_phase`, `resume_source`, and `resume_child`, then continue with the next adapter-native step: usually `/trellis-sp:specify`, `/trellis-sp:clarify`, `/trellis-sp:plan`, or `/trellis-sp:execute` depending on readiness.'
  ...
  printf '%s\n' '- Parent-task resume template: if `normalize.md` exists but the reviewed `prd.md` is not yet planning-ready, resume with `/trellis-sp:specify`; if `last_phase=replan`, resume the corrective `/trellis-sp:execute` handoff first; if `resume_source=replan` and `last_phase=execute`, continue that corrective execute wave instead of reopening `/trellis-sp:replan`; otherwise, if the parent task is already planning-ready, resume with `/trellis-sp:plan` or `/trellis-sp:execute`.'
  printf '%s\n' '- Execute resume cursor: prefer `resume_child` when it still points to a pending child, otherwise fall back to the first pending child and skip children whose `task.json.status` is already `completed` or `done`.'
